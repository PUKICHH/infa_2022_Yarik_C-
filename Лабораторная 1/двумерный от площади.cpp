...
void check(int **pos, int *pos_elem, int **arr, int n) {
    for (int i = 0; i < *pos_elem; i++) {
        if (pos[i][0] == 0 || pos[i][0] == n - 1 || pos[i][1] == 0 || pos[i][1] == n - 1) {
            delete[] pos[i];
            ...
            *pos_elem = *pos_elem - 1;
            ...
            if (arr[pos[i][0]][pos[i][1] - 1] == 1 || arr[pos[i][0]][pos[i][1] + 1] == 1 ||
                arr[pos[i][0] - 1][pos[i][1]] == 1 || arr[pos[i][0] + 1][pos[i][1]] == 1) {
                delete[] pos[i];
                for (int j = i; j < *pos_elem; j++) {
                    pos[j] = pos[j + 1];
                }
...
int test(int n, int *s) {
    ...
    std::uniform_int_distribution<int> dist1(1, (n * n) / 2);
    int x, y, z, time = 0;
    z = dist1(rng);
